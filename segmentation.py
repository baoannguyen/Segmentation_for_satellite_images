# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 23:40:50 2020
...
for c_ax, (c_id, c_df) in zip(m_axs.flatten(), full_df.groupby('image_id')):
    ...
    for _, c_row in c_df.iterrows():
        xy_vec = np.array(c_row['segmentation'])
        c_ax.plot(xy_vec[:, 0], xy_vec[:, 1], label = c_df['id_x'])
# %%
a = full_df.sample(50)
for a,b in a.groupby('image_id'):
    print(a)
    print(b['file_name'].values[0])
# %% [markdown]
    ...
    while True:
        for (_, c_df) in np.random.permutation(all_groups):
            ...
            if len(out_img)>=batch_size:
                ...
                out_img, out_seg = [], []
valid_gen = batch_img_gen(valid_df, 9)            
...
# %% [code]
seg_model.load_weights('seg_model_weights.best_2.hdf5')
m = models.load_model('full_best_model_2.h5')
t_x, t_y = next(valid_gen)
if t_x.shape[0]>16:
    t_x = t_x[:16]
    t_y = t_y[:16]
